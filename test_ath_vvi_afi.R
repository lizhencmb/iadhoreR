...
afi_gff <- file.path(ext, "Aristolochia_fimbriata.gff")
afi_fasta <- file.path(ext, "Aristolochia_fimbriata.fasta")
...
rec_afi <- recommend_parse_gff(afi_gff, afi_fasta)
...
# afi: chromosomes 1-7
afi_lists <- parse_gff(gff_file = afi_gff,
                       output_dir   = file.path(work, "afi_lists"),
                       genome_name  = "afi",
                       feature_type = "gene",
                       id_attribute = "ID",
                       chromosomes  = paste0("GWHBEBS0000000", 1:7)

)

cat("ath gene list files:\n"); print(ath_lists)
cat("vvi gene list files:\n"); print(vvi_lists)
cat("afi gene list files:\n"); print(afi_lists)

# ── Step 2: all-vs-all DIAMOND (both species together) ────────────────────────
cat("\n--- Step 2: run_diamond() ---\n")
blast_file <- file.path(work, "ath_vvi_afi.blast")
run_diamond(
  fasta_file  = c(ath_fasta, vvi_fasta, afi_fasta),
...
blast_to_families(
  ...
  gene_list_files = c(unname(ath_lists), unname(vvi_lists), unname(afi_lists))
)
...
write_iadhore_config(
  genomes     = list(ath = ath_lists, vvi = vvi_lists, afi = afi_lists),
